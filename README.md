...
This repository contains the canonical TxtTattler application, an archive of six independent AI-generated implementations, and a follow-up experiment for measuring whether granular specification reduces architecture drift.
...
| Path | Purpose |
| --- | --- |
| [`txttattler/`](txttattler/) | Canonical reference application. This is the app to build, test, and evolve. |
| [`implementations-archive/`](implementations-archive/) | Historical generated implementations and comparative analysis documents. |
| [`architecture-drift-experiment/`](architecture-drift-experiment/) | Follow-up 2 by 2 experiment comparing one-shot generation with OpenSpec-driven generation. |
| [`openspec/`](openspec/) | OpenSpec changes used to evolve the canonical reference app. |
| [`specs/`](specs/) | Historical consolidation specs for the canonical app. |
| [`original-txttattler-creation-prompt.md`](original-txttattler-creation-prompt.md) | Original prompt used to generate the archived implementations. |
...
Each subdirectory under [`implementations-archive/`](implementations-archive/) contains a fully working, independently generated Rust project:

| Directory | Generated by |
| --------- | ----------- |
| `implementations-archive/claude/` | Claude Sonnet 4.7 (Anthropic) |
| `implementations-archive/claude-opus/` | Claude Opus 4.7 (Anthropic) |
| `implementations-archive/codex/` | Codex GPT-4.4 Medium (OpenAI) |
| `implementations-archive/codex-gpt55-high/` | Codex GPT-5.5 High (OpenAI) |
| `implementations-archive/ghcopilot/` | GitHub Copilot Auto (Microsoft) |
| `implementations-archive/grok/` | Grok Build (xAI) |
...
- [`implementations-archive/analysis-codegen.by-claude.md`](implementations-archive/analysis-codegen.by-claude.md)
- [`implementations-archive/analysis-codegen.by-codex.md`](implementations-archive/analysis-codegen.by-codex.md)
- [`implementations-archive/analysis-codegen.by-copilot.md`](implementations-archive/analysis-codegen.by-copilot.md)
- [`implementations-archive/analysis-codegen.by-grok.md`](implementations-archive/analysis-codegen.by-grok.md)

Two follow-up addenda extend the study to six implementations:

- [`implementations-archive/claude-opus-addendum.md`](implementations-archive/claude-opus-addendum.md) — analysis of Claude Opus + five-way comparison
- [`implementations-archive/codex-gpt55-high-addendum.md`](implementations-archive/codex-gpt55-high-addendum.md) — analysis of Codex GPT-5.5 High + six-way comparison and final ranking
...
The canonical app preserves the feature set from the original generation spec. The archived implementations vary in coverage and quality; the analysis documents detail where each one hits or misses.
...
The canonical app is the Cargo project in `txttattler/`:

```bash
git clone https://github.com/yourname/txttattler
cd txttattler/txttattler
cargo build --release
# binary lands at target/release/txttattler
```

The archived generated implementations are still buildable from their archived paths, for example:

```bash
cd implementations-archive/ghcopilot
cargo build --release
```
